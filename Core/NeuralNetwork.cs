using MultilayerPerceptron.Core.Math;
    ...
    public class NeuralNetwork : ICloneable
    {
        ...
        public NeuralNetwork(params int[] layers)
        {
            ...
            for (int layer = 1; layer < layers.Length; layer++) 
            {
                ...
                {
                    NeuronWeights[layer - 1][weightID] = RandomGenerator.NextFloat(-0.5f, 0.5f);
                }
            }
        }
        ...
        {
            File.WriteAllText(fileName, JsonConvert.SerializeObject(Neurons) + JsonConvert.SerializeObject(NeuronWeights));
        }
        ...
        public void SetInputValues(params float[] arrayOfInputValues) 
        {
            if (arrayOfInputValues.Length != Neurons[0].Length)
            {
                throw new ArgumentException("The number of input values " +
        ...
        public void CalculateNetwork() 
        {
            for (int layer = 1; layer < Neurons.Length; layer++)
            {
                for (int neuron = 0; neuron < Neurons[layer].Length; neuron++)
                {
                    ...
                    for (int previousLayerNeuron = 0;
                             previousLayerNeuron < Neurons[layer - 1].Length;
                             previousLayerNeuron++)
                    {
            ...
            for (int layer = Neurons.Length - 1; layer > 0; layer--) // Обратное распространение ошибки
            {
                for (int neuron = 0; neuron < Neurons[layer].Length; neuron++)
                {
                    ...
                    for (int previousLayerNeuron = 0; 
                             ...
                             previousLayerNeuron++)
                    {
                        Neurons[layer - 1][previousLayerNeuron].Error
        ...
        public void MutateWeights(float mutationRange)
        {
            for (int layer = 0; layer < NeuronWeights.Length; layer++)
            {
                ...
                {
                    NeuronWeights[layer][weightID] += RandomGenerator.NextFloat(-mutationRange, mutationRange);
                }
            }
        }
